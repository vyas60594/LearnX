        ...
        <div className="flex h-screen overflow-hidden bg-white">
            ...
            <div className="no-scrollbar flex flex-1 flex-col overflow-y-auto lg:pl-72 relative">
                ...
                <main className="flex-1 bg-white px-4 pt-6 pb-20 sm:px-6 lg:px-10 lg:pt-8 w-full max-w-7xl mx-auto">
                    ...
                    <div className="mb-10 bg-white border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-[24px] p-4 lg:p-5 flex flex-col lg:flex-row flex-wrap items-center gap-5 transition-all hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
                        {/* 1. Search Bar */}
                        ...
                        <div className="flex flex-row items-center gap-3 w-full lg:w-auto overflow-x-auto no-scrollbar py-1 shrink-0">
                                    ...
                                    {PRACTICE_TESTS_CATEGORIES.map(category => (
                                        <button
                                            ...
                                            className={`relative px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl text-[10px] sm:text-xs font-bold transition-all duration-300 whitespace-nowrap overflow-hidden ${selectedCategory === category
                                                ? 'bg-transparent text-white shadow-md shadow-indigo-500/30 translate-y-[-1px]'
                            ...
                            {/* 3. Level Filter */}
                            <div className="flex gap-1 p-1.5 bg-slate-50/80 border border-slate-100 rounded-2xl shrink-0 shadow-inner">
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`relative px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl text-[10px] sm:text-xs font-bold transition-all duration-300 whitespace-nowrap overflow-hidden ${isActive
                                                ? `bg-transparent text-white shadow-md ${activeShadow} translate-y-[-1px]`
                                                : 'text-slate-500 hover:text-slate-800 hover:bg-white/60'
                                                }`}
                                        >
                                            {isActive && (
                                                <span className={`absolute inset-0 ${activeBg} rounded-xl -z-10`} />
                                            )}
                                            {level}
                                        </button>
                                    )
...
function TestCard({ test, onStart }) {
    ...
    return (
        <div className="group bg-white border border-slate-100 rounded-[32px] p-5 sm:p-8 shadow-sm hover:shadow-xl hover:border-indigo-100 transition-all duration-300 flex flex-col h-full">
            <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
                <div className="flex flex-wrap gap-2">
                    <span className={`px-4 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-wider whitespace-nowrap ${categoryColors[test.category] || 'bg-slate-100 text-slate-600'}`}>
                        {test.category}
                    </span>
                    <span className={`px-4 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-wider whitespace-nowrap ${levelColors[test.level] || 'bg-slate-100 text-slate-600'}`}>
                        {test.level}
                    </span>
                </div>
            ...
            <div className="flex flex-wrap items-center gap-x-6 gap-y-3 mb-8 mt-auto">
                <div className="flex items-center gap-2.5 text-slate-600">
                    ...
                    <span className="text-xs font-bold tracking-tight whitespace-nowrap">{test.questions?.length || 0} Questions</span>
                </div>
                <div className="flex items-center gap-2.5 text-slate-600">
                    <div className="p-1.5 bg-slate-50 rounded-lg group-hover:bg-indigo-50 transition-colors shrink-0">
                        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    <span className="text-xs font-bold tracking-tight whitespace-nowrap">{test.duration}</span>
                </div>
            </div>
